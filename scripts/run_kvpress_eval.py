...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run workload-aware KVPress evaluation sweeps.")
    ...
    parser.add_argument("--device", default="auto", help="Transformers device_map value or cpu/auto.")
    parser.add_argument("--torch-dtype", default="auto", help="Torch dtype name like float16, bfloat16, or auto.")
    parser.add_argument("--max-cases", type=int, default=None, help="Optional cap for quick smoke tests.")
    parser.add_argument(
        "--output",
        default=PROJECT_ROOT / "results" / "raw" / f"kvpress_eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
    )
    parser.add_argument("--run-name", default="kvpress_eval")
